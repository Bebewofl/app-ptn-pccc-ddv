...
Copy-Item (Join-Path $root 'hub-v224-versionfix.js') (Join-Path $work 'public\hub-v224-versionfix.js') -Force
$html=Get-Content $index -Raw -Encoding UTF8
$html=[regex]::Replace($html,'Phiên bản\s+V\d+(?:\.\d+)*','Phiên bản V2.2.4')
if($html -notmatch 'hub-v224-versionfix\.js'){
  $html=$html -replace '</body>',"  <script src=`"hub-v224-versionfix.js?v=224fix1`"></script>`r`n</body>"
}
Set-Content $index $html -Encoding UTF8

$node=Get-Command node -ErrorAction SilentlyContinue
if($node){
  & $node.Source --check (Join-Path $work 'public\hub-v224-versionfix.js')
  if($LASTEXITCODE -ne 0){throw 'hub-v224-versionfix.js syntax failed.'}
  Write-Host 'Version fix JavaScript syntax: OK' -ForegroundColor Green
}

# Recreate the output ZIP so the downloaded package also contains the corrected visible version.
...
# Redeploy the SAME preview channel only. Production remains unchanged.
$fb=$null
...
Write-Host 'VISIBLE VERSION FIX DEPLOYED: HUB V2.2.4. Production unchanged.' -ForegroundColor Green
