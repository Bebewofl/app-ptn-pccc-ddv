/* Interdepartment workflow, consolidated from V2.2.1 and V2.2.3. */
(function(){
  ...
  function warm(list){
    (list||[]).forEach(c=>{
      ...
      const generation=sessionGeneration;
      WARM.add(id);setTimeout(()=>{if(generation===sessionGeneration)loadComments(id).finally(()=>WARM.delete(id))},0);
    });
  }
  ...
  function renderBoard(){
    ...
    return '<div class="caseboard">'+buckets.map(([title,fn])=>{const rows=list.filter(fn);return '<div class="casecol"><h3>'+esc(title)+'</h3>'+ (rows.length?rows.map(card).join(''):'<div class="v22-empty">Không có.</div>')+'</div>';}).join('')+'</div>';
  }
  ...
  async function grantRndHead(){
    ...
    try{
      ...
      alert('Đã lưu quyền Trưởng phòng R&D. Cần đăng nhập lại và kiểm tra phạm vi truy cập trên Preview.');
  ...
  function responseHtml(r){
    const files=attachmentLinks(r.attachments);const links=(r.externalLinks||[]).map(u=>`<a class="v22-attachment" href="${attr(safeLink(u))}" target="_blank" rel="noopener">🔗 Tài liệu liên kết</a>`).join('');
  ...
  async function injectPanel(caseId){
    ...
    await loadComments(caseId);if(slot.isConnected)slot.innerHTML=panelHtml(c);
  }
  ...
  async function addEvent(caseId,action,detail){
    try{const a=authorFields();await db.collection('hub_case_events').add({caseId:String(caseId),action,detail:detail||'',actorUid:a.userUid,actorEmail:a.userEmail,actorName:a.userName,createdByUid:a.userUid,createdByEmail:a.userEmail,createdAt:FV.serverTimestamp(),visibility:'public',schemaVersion:VERSION});return true}catch(e){console.warn('Audit event not saved',e?.code||e?.message||e);return false}
  }
  ...
  window.v22SaveResponse=async function(caseId){
    ...
    try{
      const attachments=await buildAttachments(Array.from(document.getElementById('v22Files_'+sid)?.files||[]).slice(0,3),[]),a=authorFields();
      ...
      const audited=await addEvent(caseId,'DEPARTMENT_RESPONSE',`${unitLabel(currentUnit(c))}: ${status}`);
      CACHE.delete(String(caseId));
      alert(audited?'Đã gửi phản hồi và lưu truy vết.':'Đã gửi phản hồi nhưng chưa lưu được audit. Vui lòng báo quản trị viên.');
      setTimeout(()=>{try{openCaseDetail(String(caseId))}catch(e){}},120);
    }catch(e){
      const msg=String(e?.message||e);
      if(/permission|insufficient/i.test(msg))alert('Phản hồi chưa được lưu vì quyền truy cập hiện tại. Vui lòng báo quản trị viên kiểm tra quyền của VM này.');
  ...
  async function sendChat(caseId){
    ...
    try{await db.collection('hub_comments').add({caseId:String(caseId),type:'interdept_chat',message:msg,userUid:currentUser.uid,userEmail:email(),userName:currentUser.displayName||email(),createdAt:FV.serverTimestamp(),schemaVersion:VERSION});input.value='';await loadComments(caseId);await injectPanel(caseId)}catch(e){alert('Không gửi được chat: '+(e?.message||e))}
  }
  ...
  function appendCreateAttachments(){
    const body=document.getElementById('drawerBody');
    const box=document.createElement('div');box.className='v22-form';box.id='v22CreateAttachments';
    box.innerHTML='<h4>TỆP / ẢNH / TÀI LIỆU ĐÍNH KÈM</h4><div class="v22-field"><label>File / ảnh nhỏ</label><input id="cV22Files" type="file" multiple></div><div class="v22-field"><label>Thư mục</label><input id="cV22Folder" type="file" multiple webkitdirectory directory></div><div class="v22-field"><label>Link tài liệu lớn (Drive/OneDrive)</label><input id="cV22Link" placeholder="https://..."></div>';
    body.appendChild(box);
  }
  async function prepareCreateAttachments(){
    const link=document.getElementById('cV22Link')?.value.trim()||'';
    if(link&&!/^https?:\/\//i.test(link))throw Error('Link phải bắt đầu bằng http:// hoặc https://');
    const attachments=await buildAttachments(Array.from(document.getElementById('cV22Files')?.files||[]),Array.from(document.getElementById('cV22Folder')?.files||[]));
    return {attachments,externalLinks:link?[link]:[]};
  }
  async function saveCreateAttachments(caseId,data){
    if(!data.attachments.length&&!data.externalLinks.length)return;
    await db.collection('hub_comments').add({caseId,type:'case_attachment',...data,...authorFields(),createdAt:FV.serverTimestamp(),schemaVersion:VERSION});
    CACHE.delete(String(caseId));
  }
  function safeLink(value){try{const u=new URL(String(value));return ['http:','https:'].includes(u.protocol)?u.href:''}catch{return ''}}
  function reset(){sessionGeneration++;CACHE.clear();WARM.clear()}
  window.HUB_INTERDEPT={isRndHead,render:renderInterdept,renderBoard,injectPanel,appendCreateAttachments,prepareCreateAttachments,saveCreateAttachments,reset};
